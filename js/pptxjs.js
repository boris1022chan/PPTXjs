...
function pptxToHtml(options) {
    var $result = document.getElementById(options.divId);
    ...
    function convertToHtml(file) {
        ...
        if (document.getElementById("all_slides_warpper") === null) {
            ...
            allSlidesWrapper.id = "all_slides_warpper";
            allSlidesWrapper.className = "slides";
            slides.forEach(slide => {
                allSlidesWrapper.appendChild(slide);
            });
            $result.appendChild(allSlidesWrapper);
        }
    }
    ...
    function readXmlFile(zip, filename, isSlideContent) {
        try {
            ...
            if (isSlideContent && app_version <= 12) {
                //< office2007
                //remove "<![CDATA[ ... ]]>" tag
    ...
    function getSlideSizeAndSetDefaultTextStyle(zip) {
        ...
        var app_version_str = app["Properties"]["AppVersion"]
        app_version = parseInt(app_version_str);
        console.log("create by Office PowerPoint app verssion: ", app_version_str)
        ...
        for (var i = 0; i < prgrphs_arry.length; i++) {
            ...
            if (buSpan.length > 0) {
                ...
                for (var j = 0; j < buSpan.length; j++) {
                    var bult_typ = buSpan[j].dataset.bulltname;
                    var bult_lvl = buSpan[j].dataset.bulltlvl;
